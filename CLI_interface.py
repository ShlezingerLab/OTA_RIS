...
def _build_cli_runs(
    ...
    for sweep_v in sweep_vals or [None]:
        ...
        # Resolve common path arguments to absolute paths (always)
        for path_arg in ["--teacher_path", "--load_path", "--classifier_path", "--load_classifier_path", "--load_encoder", "--load_ctrl", "--save_path", "--save_encoder", "--save_ctrl", "--save_decoder"]:
            if path_arg in args:
                try:
...
if __name__ == "__main__":
    ...
    IDE_TRAIN_ARGS: dict[str, object] = {
        ...
        # "--teacher_use_channel":  teacher_use_channel#CNN teacher
    }
    if bottleneck_dim is not None:
        IDE_TRAIN_ARGS["--bottleneck_dim"] = bottleneck_dim

    # Select the stage configuration based on IDE_TRAIN_STAGE
    teacher_path = f"models_dict/teacher_{teacher_type}_{train_mode}.pth"
    ...
    STAGED_CONFIGS = {
        ...
        0.5: { # PHASE 0.5: Complexity Shift (Fine-tuning Teacher)
                "--load_classifier_path": "models_dict/teacher_cnn_full.pth", # Original heavy teacher
                "--classifier_path": "models_dict/teacher_cnn_full_shifted.pth",    # New simplified teacher
...
######################################################################
    ...
    if not argv:
        ...
        if IDE_COMMAND == "train" and IDE_TRAIN_STAGE == 4:
            ...
            # Skip controller training (Stage 2) if combine_mode is "direct"
            combine_mode = IDE_TRAIN_ARGS.get("--combine_mode", "both")
            if isinstance(combine_mode, str) and combine_mode.lower() == "direct":
                stages_to_run = [1, 3]  # Skip Stage 2 (controller not needed for direct path)
                print(f"[INFO] combine_mode='direct': Skipping Stage 2 (controller training)")
        else:
        ...
        # Default to the configured teacher_type so stage 0/0.5 works as expected.
        teachers_to_train = IDE_TRAIN_ARGS.get("--teacher_type_train", teacher_type)
        ...
        for stage in stages_to_run:
            # Phase 0 & 0.5: Train/Fine-tune teachers specified in teacher_type_train
            if stage in [0, 0.5]:
                print(f"\n>>> [IDE] Stage {stage}: Processing teachers: {teachers_to_train}")
                for t_type in teachers_to_train:
                    print(f"\n>>> [IDE] Processing teacher type: {t_type}")
                    config = dict(IDE_TRAIN_ARGS)

                    # If we are doing a combined run or specific stage,
                    # we might need to update config with stage-specific settings
                    ...
                    # Get plot path from config
                    plot_path_from_config = config.get("--plot_path", "")

                    if str(t_type).lower() == "cnn":
                        default_p = teacher_path if teacher_type == 'cnn' else "models_dict/teacher_cnn_demo.pth"
                        # Prioritize paths set in STAGED_CONFIGS
                        c_path = config.get("--classifier_path", default_p)
                        l_path = config.get("--load_classifier_path") # This will be None if not in 0.5
                        ...
                        if "--plot_path" not in config:
                            config["--plot_path"] = _resolve_generic_path(plot_path_from_config) if plot_path_from_config else _resolve_generic_path(f"plots/phase{stage}_cnn.png")
                        ...
                        if "--plot_path" not in config:
                            config["--plot_path"] = _resolve_generic_path(plot_path_from_config) if plot_path_from_config else _resolve_generic_path(f"plots/phase{stage}_heavy_intermediate.png")
                        ...
                        config.update({
                            ...
                            "--plot_path": _resolve_generic_path(plot_path_from_config) if plot_path_from_config else _resolve_generic_path(f"plots/teacher_e2e.png"),
                        })
                    ...
                    for k, v in config.items():
                        ...
                        # Skip internal playground flags
                        if k in {"--encoder_teacher_type", "--controller_teacher_type", "--teacher_type_train"}:
                            continue

                        # Explicitly handle boolean flags (no value)
                        if k in {"--train_classifier", "--complexity_shift", "--load_classifier", "--teacher_use_channel", "--grad_approx", "--cotrl_CSI", "--cotrl_signal"}:
                            if bool(v):
                                simple_args.append(k)
                            continue

                        # Skip specific save arguments that are already handled by --classifier_path or --save_path
                        if k in {"--save_encoder", "--save_ctrl", "--save_decoder"}:
                            continue

                        # Handle lists/tuples (if any)
                    ...
                    if rc != 0: break
                if rc != 0: break
                continue # Finished Stage 0 or 0.5
