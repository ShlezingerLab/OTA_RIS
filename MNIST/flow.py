...
class Encoder(nn.Module):
    def __init__(self, Nt):
        ...
        self.encoder = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Flatten(),
            nn.Linear(128 * (28 // 8) * (28 // 8), 1 * 2*self.Nt),
        )
    def forward(self, x):
        encoded = self.encoder(x)
        encoded = encoded.view(-1, 1, 2*self.Nt)
...
class Encoder_old(nn.Module):
    def __init__(self, out_dim=128, power=1.0):
        """
        Args:
            out_dim: Output dimension (N_t)
            power: Transmit power P in Watts (default 1.0W = 30 dBm from article)
        """
        super().__init__()
        self.power = power  # transmit power P (in Watts)
        # 3× Conv layers
