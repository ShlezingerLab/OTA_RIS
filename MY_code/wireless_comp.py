...
def build_theta_net(input_dim, hidden_dim, output_dim, depth=8, dropout=0.1):
    layers = [nn.LayerNorm(input_dim), nn.Linear(input_dim, hidden_dim), nn.GELU(), nn.Dropout(dropout)]
...
def test_physical_channel(teacher_suffix, timestamp, SNR):
    ...
    power = 1.0
    batch_size = 100
    channel_sampling_size = 20  # Number of different channels to cycle through
    noise_std = 1e-3
    device = "cuda" if torch.cuda.is_available() else "cpu"
    # ---------------- Load Model & Data ---------------- #
    teacher = MyTeacher(n_t=N_t, n_r=N_r, n_m=N_m, num_classes=num_classes, power=power).to(device)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    model_path = os.path.join(script_dir, "simulations", f"{timestamp}", f"{teacher_suffix}.pth")
    ...
    theta_net_path = os.path.join(script_dir, "simulations", "20260410_1740", "theta_net_frobenius.pth")
    theta_ckpt = torch.load(theta_net_path, map_location=device)
    theta_state = (
        theta_ckpt["theta_net_state_dict"]
        if isinstance(theta_ckpt, dict) and "theta_net_state_dict" in theta_ckpt
        else theta_ckpt
    )
    input_dim = int(theta_ckpt.get("input_dim", 2 * N_m * N_t + 2 * N_r * N_m))
    hidden_dim = int(theta_ckpt.get("hidden_dim", 1024))
    theta_depth = int(theta_ckpt.get("theta_depth", 8))
    theta_dropout = float(theta_ckpt.get("theta_dropout", 0.0))
    theta_net = build_theta_net(
        input_dim=input_dim,
    ...
    with torch.no_grad():
        ...
        for images, labels in test_loader:
            ...
            sample_idx += B
            with torch.no_grad():
                #phi_opt = teacher._optimize_phi_gd(s, y_learned, H_1_batched, H_2_batched,iters=20)
                h1_real = torch.view_as_real(H_1_batched).reshape(B, -1)  # (B, 2*Nm*Nt)
                h2_real = torch.view_as_real(H_2_batched).reshape(B, -1)  # (B, 2*Nr*Nm)
                x_theta = torch.cat([h1_real, h2_real], dim=1)
                theta = theta_net(x_theta)  # (B, Nm)
                phi_opt = torch.exp(1j * theta)
            ...
            #inner_prod = (y_physical.conj() * y_learned).sum(dim=1)
            #cos_sim = inner_prod / (torch.norm(y_physical, dim=1) * torch.norm(y_learned, dim=1))
...
if __name__ == "__main__":
    timestamp, SNR = "20260410_1711", 30.0
    teacher_suffix = "teacher_debug_target_snr_db=0.0"
    ...
    # script_dir = os.path.dirname(os.path.abspath(__file__))
    # plot_path = os.path.join(script_dir, "simulations", f"debug.png")
